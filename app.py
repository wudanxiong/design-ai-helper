import streamlit as st
import pandas as pd
import numpy as np
from openai import OpenAI
# 使用轻量级 OCR 引擎，适配云端环境
from rapidocr_onnxruntime import RapidOCR
import pdfplumber
from docx import Document
from docx.shared import Pt, RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
import io
import json
import re
from datetime import datetime

# ================= 配置区 =================

# 🔴🔴🔴 请在此处填入您的 DeepSeek API Key 🔴🔴🔴
# 建议使用 st.secrets 更安全，或者直接替换下方的字符串
API_KEY = "sk-90ed7f3913324b6391f15805f0f963ea" 

# DeepSeek 官方 API 地址
BASE_URL = "https://api.deepseek.com"

# 设置页面
st.set_page_config(page_title="装修规划云助手", layout="wide")

# ================= 初始化 OCR =================
@st.cache_resource
def load_ocr():
    # RapidOCR 初始化非常快，不需要下载大模型
    engine = RapidOCR()
    return engine

try:
    ocr = load_ocr()
except Exception as e:
    st.error(f"OCR 引擎初始化失败: {e}")
    ocr = None

# ================= 预设长文本内容 (完整版) =================
TEXT_BLOCKS = {
    "flow_1": """推荐【1号流程】（第一步:从抖音、小红书、初步筛选感觉靠谱有经验的设计师
（独立设计师优点：能拿到你全部或大部分设计费，装修公司的设计师要和公司平分，但当地的独立设计师很少，且很贵，如果不是别墅大平层，做很复杂的硬装，可以考虑外地的设计师）
注意1：专业设计师一定是先付费才会给你做以及用心去做方案。
注意2：如果对接的是销售不需要聊太多，要求直接对接设计师就行，去跟具体设计师聊。
第二步:挑选感觉靠谱的问五个问题
这五个问题可以文字信息沟通，本着对自己负责的态度请务必了解清楚设计师全部信息。
①是本人来设计吗？
②能签设计合同吗？(不能的过)
③设计费多少一平，按什么算(按套内或建筑外轮廓合理)；包含多少图纸(平面布局，效果图概念方案，效果图，全套施工图)；用什么软件做的 （都可以，但用酷家乐做的便宜）
④是否科班出身？干了几年？学过美术没？
⑤能否按模板报价出报价单(到时候给他我的报价模板)

注意：可以先说：我家里对设计有一定要求，想找个专业有经验且能够根据我的预算帮我做专业规划的设计师，而且想先签设计合同先做设计方案再考虑施工的事。

第三步:如果回答的都可以，用我的设计专业度提问表进行提问(一定是打电话提问，可以说可以电话沟通一下吗，有几个问题想请教一下)，对于问题的回答率准确率至少要在60%以上，否则就筛掉。

第四步: 最后筛选2-3个合格者要2~3套平面➕效果图➕cad施工图(必须是对起来的一套房子的成套的图纸)，发来我看。
发的时候请先发设计师情况，然后再发设计师作品。

第五步:我看过方案ok后，签独立设计合同，做装修标准。
标准一：设计施工标准(深度平面布局图、效果图概念方案，所有有设计的区域的效果图、完整施工图)
标准二：报价标准，就是基础装修报价单(可直接用我的报价模板出)

第六步：找公司进行初步筛选，通过6个点筛选3~5家
①查资质（查营业执照，成立时间，有没有纠纷，用企查查）
②查规模（小公司/设计工作室规模一般在10人一下，50人以上可算作大公司，中间的是小公司）
③施工承包方式（优先选择能直接安排常年合作工人施工的）
注：到时候可以再详细问我
④问付款方式（选择先施工后付款或首付不超过20%的）
⑤问合同是否可以修改（或者直接用自己的签）
⑥看工地(看水电阶段，木工未封完石膏板阶段，完工阶段)
注：看工地可以拍视频给我
第七步:
把设计方案和标准报价发给装修公司，追求性价比就找中小公司直接对接老板(省去了给员工的业务提成)，让他们在完全统一的标准下去填价格，让他直接抛去给设计师提成，然后来横向对比各家价格。""",

    "flow_2": """推荐【2号流程】（第一步：找公司进行初步筛选，通过6个点筛选3~5家
先问下能否签独立的设计合同
①查资质（查营业执照，成立时间，有没有纠纷，用企查查）
②看规模（优先选择干了5年以上的中小公司）
③施工承包方式（优先选择能直接安排常年合作工人施工的）
注：到时候可以再详细问我
④问付款方式（选择先施工后付款或首付不超过20%的）
⑤问合同是否可以修改（或者直接用自己的签）
⑥看工地(看水电阶段，木工未封完石膏板阶段，完工阶段)
注：看工地可以拍视频给我
第二步:要求装修公司安排有充足设计经验、懂材料工艺的设计师对接问五个问题
这五个问题可以文字信息沟通，本着对自己负责的态度请务必了解清楚设计师全部信息。
①是本人来设计吗？还是外包
②设计费多少一平，按什么算(按套内或建筑外轮廓合理)；包含多少图纸(平面布局，效果图概念方案，效果图，全套施工图)；用什么软件做的 （都可以，但用酷家乐做的便宜）
③是否科班出身？干了几年？学过美术没？
④能否按模板报价出报价单(到时候给他我的报价模板)
注意：可以先说：我家里对设计有一定要求，想找个专业有经验且能够根据我的预算帮我做专业规划的设计师，而且想先签设计合同先做设计方案再考虑施工的事。

第三步:如果回答的都可以，则用我的设计专业度提问表进行提问(一定是打电话提问，可以说可以电话沟通一下吗，有几个问题想请教一下)，对于问题的回答率准确率至少要在60%以上，否则就筛掉。

第四步: 最后筛选2-3个合格者要2~3套平面➕效果图➕cad施工图(必须是对起来的一套房子的成套的图纸)，发来我看。
发的时候请先发设计师情况，然后再发设计师作品。

第五步:我看过方案ok后，签独立设计合同，做装修标准。
标准一：设计施工标准(深度平面布局图、效果图概念方案，所有有设计的区域的效果图、完整施工图)
标准二：报价标准，就是基础装修报价单(可直接用我的报价模板出)

第六步:
把设计方案和标准报价发给装修公司，追求性价比就找中小公司直接对接老板(省去了给员工的业务提成)，让他们在完全统一的标准下去填价格，让他直接抛去给设计师提成，然后来横向对比各家价格。）""",

    "flow_3": """推荐【3号流程】（第一步：找公司进行初步筛选，通过6个点筛选35家
第一步：找公司进行初步筛选，通过6个点筛选35家并先问下能否签独立的设计合同
①查资质（查营业执照，成立时间，有没有纠纷，用企查查）
②看规模（优先选择干了5年以上的中小公司）
③施工承包方式（优先选择能直接安排常年合作工人施工的）
注：到时候可以再详细问我
④问付款方式（选择先施工后付款或首付不超过20%的）
⑤问合同是否可以修改（或者直接用自己的签）
⑥看工地(看水电阶段，木工未封完石膏板阶段，完工阶段)
注：看工地可以拍视频给我，直接确定好装修公司
第二步:要求装修公司安排有设计经验、懂材料工艺的设计师对接。
第三步: 交不超过1000的定金，出平面图和效果图。 并要求用我的报价模板出报价。
第四步:做完平面、效果图、报价，每做完一个发我审一下。
第五步：都为问题了再签施工合同。""",

    "mat_3": """推荐【3号辅材表】：
(电线:正泰/普瑞斯曼低烟无卤电线; 网线：大唐电信/泛达/飞利浦等六类网线+光纤; 空气开关:施耐德; 穿线管底盒：伟星/日丰/中财(重型线管)； 水管:管径25德国微法/阔盛（顶部水管用金属固定件）; 石膏板龙骨：可耐福/圣戈班； 白乳胶：小西/百得； 欧松板:进口爱格F4; 防水:马贝; 瓷砖胶：马贝； 水泥砂浆：马贝成品预拌砂浆； 拉毛乳液：马贝sp700；墙固：马贝G； 腻子粉石膏粉：美巢/圣戈班； 乳胶漆:纯进口乳胶漆（美国大师至臻/都芳布兰奇）； 自流平：马贝； 植筋胶：悍马；下水管阻尼片：大能；""",
    
    "mat_2": """推荐【2号辅材表】：
(电线：当地大厂的/特变普通BV线； 空气开关：正泰； 网线：大唐电信/泛达/飞利浦等六类网线；穿线管底盒：伟星/日丰/中财； 水管：伟星/日丰（顶部水管用金属固定件）； 石膏板龙骨：可耐福/圣戈班； 白乳胶：小西/百得； 基层欧松板： 巴西LP/进口爱格f4； 防水：伟星咖乐/德高墙刚地柔； 瓷砖胶：德高/雨虹； 拉毛乳液：马贝sp700； 墙固：马贝G； 腻子粉石膏粉：美巢/圣戈班（南方有回南天的考虑用耐水腻子）； 乳胶漆：多乐士专业生态无添加金钻全效/致悦抗甲醛净味五合一/立邦云净竹炭抗甲醛五合一或同档次知名品牌乳胶漆。)""",

    "mat_1": """推荐【1号辅材表】：
(电线：当地大厂的或者特变普通BV线; 空气开关：德力西; 网线：大唐电信/泛达/飞利浦超五类网线; 穿线管底盒：联塑/保利/伟星; 水管：联塑/保利/伟星; 石膏板：泰山普通纸面石膏板; 龙骨：国标龙骨; 基层欧松板：千年舟/万华/鲁丽e0; 防水：伟星咖乐/德高墙刚地柔; 瓷砖胶：德高/雨虹; 墙固：立邦/多乐士墙固; 腻子粉石膏粉：立邦/多乐士/祥得士（南方有回南天的考虑用耐水腻子）; 乳胶漆：多乐士专业生态无添加金钻全效/致悦抗甲醛净味五合一/立邦云净竹炭抗甲醛五合一或同档次知名品牌乳胶漆。)"""
}

# ================= 核心功能函数 =================

def extract_text_from_file(uploaded_file):
    """根据文件类型提取文本内容 (适配 RapidOCR)"""
    if ocr is None:
        return "OCR 引擎未就绪"

    text_content = ""
    file_type = uploaded_file.name.split('.')[-1].lower()
    
    try:
        if file_type in ['jpg', 'jpeg', 'png']:
            # 图片 OCR
            bytes_data = uploaded_file.getvalue()
            # RapidOCR 直接处理二进制流
            result, _ = ocr(bytes_data)
            if result:
                # result 格式: [[box, text, score], ...]
                for line in result:
                    text_content += line[1] + "\n"
                    
        elif file_type == 'pdf':
            # PDF 处理
            with pdfplumber.open(uploaded_file) as pdf:
                for page in pdf.pages:
                    page_text = page.extract_text()
                    if page_text and len(page_text) > 30:
                        text_content += page_text + "\n"
                    else:
                        # 扫描版 PDF 转图片 OCR
                        im = page.to_image(resolution=200) # 200dpi 足够且快
                        img_byte_arr = io.BytesIO()
                        im.original.save(img_byte_arr, format='PNG')
                        result, _ = ocr(img_byte_arr.getvalue())
                        if result:
                            for line in result:
                                text_content += line[1] + "\n"

        elif file_type == 'docx':
            doc = Document(uploaded_file)
            for para in doc.paragraphs:
                text_content += para.text + "\n"
            for table in doc.tables:
                for row in table.rows:
                    for cell in row.cells:
                        text_content += cell.text + " "
                    text_content += "\n"

        elif file_type in ['txt']:
            text_content = uploaded_file.getvalue().decode("utf-8")
        
        elif file_type in ['doc', 'wps']:
            st.warning(f"检测到 {file_type} 格式。云端环境可能无法完美解析旧版二进制格式。")
            try:
                text_content = uploaded_file.getvalue().decode("gbk", errors='ignore')
            except:
                text_content = uploaded_file.getvalue().decode("utf-8", errors='ignore')

    except Exception as e:
        st.error(f"文件读取错误: {str(e)}")
        
    return text_content

def analyze_data_with_llm(raw_text):
    """调用 DeepSeek API 分析"""
    
    if "sk-" not in API_KEY:
        st.error("请先在代码中填入正确的 DeepSeek API Key！")
        return None

    prompt = f"""
    你是一个专业的数据提取助手。请阅读以下装修调查表的内容，提取关键数据并以JSON格式返回。
    如果用户未提及某项，请在JSON值中返回 null。不要臆造数据。
    
    需要提取的字段（字段名必须严格一致）：
    1. hard_budget (硬装预算，数字，单位元)
    2. total_budget (整体入住预算，数字，单位元)
    3. area (建筑面积，数字，单位平方米)
    4. score (用户对装修效果的评分要求，1-10分，如果没有明确分数但有描述，9-10为高要求/完美，7-8为中高，5-6为普通/及格，5以下为无所谓)
    5. has_paid_design (是否已付费设计，true/false)
    6. personality_trouble (是否怕麻烦，选项："怕麻烦" 或 "不怕麻烦")
    7. personality_cautious (是否谨慎，选项："谨慎" 或 "敢放手")
    8. personality_perfect (是否追求完美/平衡/性价比，选项："追求完美", "追求平衡", "追求性价比")
    9. weekly_free_time (每周空闲时间，数字，单位天)
    10. house_is_old (是否老房，true/false)
    11. house_age (房龄，数字，年)
    12. ventilation (通风晾晒条件，选项："好", "晾晒大于1年", "晾晒小于1年", "差")
    13. willing_to_self_build (是否愿意自装，true/false)
    14. budget_can_cover_design (是否接受设计费，true/false，默认为true除非明确拒绝)

    原文内容：
    {raw_text[:3500]} 
    """
    
    try:
        client = OpenAI(api_key=API_KEY, base_url=BASE_URL)
        response = client.chat.completions.create(
            model="deepseek-chat", # DeepSeek V3
            messages=[
                {"role": "system", "content": "You are a helpful assistant that outputs strictly JSON."},
                {"role": "user", "content": prompt},
            ],
            response_format={ "type": "json_object" }
        )
        
        json_str = response.choices[0].message.content
        data = json.loads(json_str)
        return data
    except Exception as e:
        st.error(f"AI 分析失败 (API调用错误): {e}")
        return None

def logic_engine(data):
    """基于规则的专家系统逻辑 (完整版)"""
    result = {}
    
    # 辅助函数：处理缺失数据
    def get_val(key, default=None):
        val = data.get(key)
        if val is None:
            return None
        return val

    # 提取数据
    hard_budget = get_val('hard_budget')
    total_budget = get_val('total_budget')
    area = get_val('area')
    score = get_val('score')
    has_paid_design = get_val('has_paid_design')
    
    # 逻辑一：预算是否合理
    res_1 = "客户未提供数据"
    if hard_budget and total_budget:
        ratio = hard_budget / total_budget
        if ratio >= 0.5:
            res_1 = "合理"
        elif ratio < 0.5 and (0.5 - ratio) > 0.2: 
            if ratio < 0.3:
                 res_1 = "不合理。建议：硬装预算通常应占整体预算的45%-55%左右。"
            else:
                 res_1 = "基本合理（偏低）"
        else:
             res_1 = "不合理。建议：硬装预算通常应占整体预算的45%-55%左右。"
    result['budget_analysis'] = res_1

    # 逻辑二：设计费预算建议
    res_2 = "客户未提供数据"
    design_unit_price = 0
    
    if has_paid_design:
        res_2 = "（留白：客户已做付费设计）"
    elif hard_budget and area and score:
        hard_unit_price = hard_budget / area
        design_fee_str = ""
        
        # 判定预算区间
        if 1300 <= hard_unit_price: 
            if area > 80 and score >= 9:
                design_fee_str = f"{int(hard_budget*0.10)} ~ {int(hard_budget*0.12)} 元"
            elif area > 80 and 7 <= score <= 8:
                design_fee_str = f"{int(hard_budget*0.08)} ~ {int(hard_budget*0.10)} 元"
            else:
                 design_fee_str = "建议预留硬装预算的8%-10%" 
        
        elif 900 <= hard_unit_price < 1300: 
            if score >= 9:
                 design_fee_str = f"{int(hard_budget*0.10)} ~ {int(hard_budget*0.11)} 元"
            elif 7 <= score <= 8:
                 design_fee_str = f"{int(hard_budget*0.08)} ~ {int(hard_budget*0.09)} 元"
            else:
                 design_fee_str = "建议预留硬装预算的7%-8%"

        elif 500 <= hard_unit_price < 900: 
            if score >= 9:
                design_fee_str = f"{int(hard_budget*0.09)} ~ {int(hard_budget*0.10)} 元"
            elif 7 <= score <= 8:
                design_fee_str = f"{int(hard_budget*0.07)} ~ {int(hard_budget*0.08)} 元"
            elif 5 <= score <= 6:
                design_fee_str = f"{int(hard_budget*0.05)} ~ {int(hard_budget*0.07)} 元"
            else:
                 design_fee_str = "建议预留硬装预算的5%左右"

        elif hard_unit_price < 500: 
            if 7 <= score <= 8:
                design_fee_str = f"{int(hard_budget*0.07)} ~ {int(hard_budget*0.08)} 元"
            elif 5 <= score <= 6:
                design_fee_str = f"{int(hard_budget*0.05)} ~ {int(hard_budget*0.07)} 元"
            elif score < 5:
                design_fee_str = f"{int(hard_budget*0.03)} ~ {int(hard_budget*0.04)} 元"
            else:
                 design_fee_str = "建议预留硬装预算的3%-5%"
        
        if design_fee_str:
            res_2 = design_fee_str
            try:
                nums = re.findall(r'\d+', design_fee_str)
                if len(nums) >= 2:
                    avg_total = (float(nums[0]) + float(nums[1])) / 2
                    design_unit_price = avg_total / area
            except:
                pass
    
    result['design_fee'] = res_2

    # 逻辑三：设计师档次建议
    res_3 = "需先确定设计费预算"
    if design_unit_price > 0:
        dup = design_unit_price
        if dup >= 160:
            res_3 = "适合找高端设计师（10年以上经验，科班），建议找当地或网上独立设计师。"
        elif 80 <= dup < 160:
            res_3 = "适合找高端设计师，建议找网上独立设计师、当地小公司/工作室老板或全案员工。"
        elif 50 <= dup < 80:
            res_3 = "适合找高端设计师，建议找当地小公司/工作室老板或装修公司老员工（非营销型）。"
        elif 30 <= dup < 50:
            res_3 = "建议找当地小公司/工作室老板或装修公司老员工。"
        else:
             res_3 = "建议找注重性价比的装修公司设计或自学设计。"
    result['designer_level'] = res_3

    # 逻辑四：施工承包方式
    res_4 = "客户未提供足够判断数据（需预算、性格倾向）"
    p_trouble = get_val('personality_trouble') 
    p_cautious = get_val('personality_cautious')
    p_perfect = get_val('personality_perfect')
    weekly_time = get_val('weekly_free_time', 0) or 0
    willing_self = get_val('willing_to_self_build', False)

    if hard_budget and area:
        hup = hard_budget / area
        
        # 高预算
        if hup >= 1300:
            if p_trouble == "怕麻烦":
                res_4 = "全案落地。"
            elif p_trouble == "不怕麻烦":
                if p_cautious == "谨慎":
                    res_4 = "全案落地 或 小全包。"
                else: 
                    res_4 = "全案落地 或 小全包。"
        
        # 中高预算
        elif 900 <= hup < 1300:
            if p_trouble == "怕麻烦":
                res_4 = "全包 或 小全包。"
            elif p_trouble == "不怕麻烦" and weekly_time >= 2:
                res_4 = "半包 或 小全包。"
            else:
                res_4 = "建议全包（因时间或性格原因）。"

        # 中预算
        elif 500 <= hup < 900:
            if p_trouble == "怕麻烦" and p_cautious == "谨慎":
                res_4 = "全包 或 小全包。"
            elif p_trouble == "不怕麻烦" and p_cautious == "谨慎" and weekly_time >= 2:
                res_4 = "半包 或 小全包。"
            else:
                 res_4 = "建议全包。"

        # 低预算
        elif hup < 500:
            can_self_build = willing_self and weekly_time >= 3
            if can_self_build:
                res_4 = "可以自装。"
            elif p_cautious == "敢放手" and p_trouble == "怕麻烦":
                res_4 = "小全包。"
            elif p_cautious == "敢放手" and p_trouble == "不怕麻烦":
                res_4 = "半包。"
            else:
                res_4 = "建议半包。"
    
    result['construction_mode'] = res_4

    # 逻辑五：施工方建议
    res_5 = "客户未提供预算或追求偏好"
    if hard_budget and area and p_perfect:
        hup = hard_budget / area
        if hup >= 900:
            if p_perfect == "追求完美" or p_perfect == "追求平衡":
                res_5 = "5年以上全案落地或精工施工。"
            elif p_perfect == "追求性价比":
                res_5 = "5年以上中小公司或大型连锁。"
        elif 500 <= hup < 900:
            if p_perfect == "追求完美":
                res_5 = "大型全国连锁。"
            elif p_perfect == "追求平衡":
                res_5 = "5年以上中小装修公司。"
            else:
                 res_5 = "5年以上小装修公司。"
        else:
             res_5 = "5年以上小装修公司。"
    result['contractor'] = res_5

    # 逻辑六：辅材推荐
    res_6 = "暂无建议"
    if hard_budget and area:
        hup = hard_budget / area
        if hup >= 1300:
            res_6 = TEXT_BLOCKS['mat_3']
        elif hup >= 500:
            res_6 = TEXT_BLOCKS['mat_2']
        else:
            res_6 = TEXT_BLOCKS['mat_1']
    result['materials'] = res_6

    # 逻辑七：零冷水建议
    res_7 = "客户未提供面积数据"
    if hard_budget and area:
        hup = hard_budget / area
        if hup >= 900: 
            res_7 = "建议做大循环零冷水。"
        elif 500 <= hup < 900: 
            if area >= 80:
                res_7 = "建议做大循环。"
            else:
                res_7 = "建议做小循环。"
        else: 
            if area >= 80:
                res_7 = "建议做小循环。"
            else:
                res_7 = "建议不做零冷水。"
    result['zero_cold_water'] = res_7

    # 逻辑八：水电改造建议
    res_8 = "需提供房龄信息"
    is_old = get_val('house_is_old')
    h_age = get_val('house_age', 0) or 0
    
    if is_old and (h_age > 8 or (h_age == 0 and is_old)):
        res_8 = "全改。"
    elif not is_old: 
        if hard_budget and area and (hard_budget/area) >= 900:
            res_8 = "水全改，电半改或全改。"
        else:
            res_8 = "水电半改。"
    elif is_old and h_age <= 8:
         if hard_budget and area and (hard_budget/area) < 900:
             res_8 = "水全改，电部分半改(厨卫空调全改)。"
         else:
             res_8 = "建议全改（预算充足情况）。"
    
    result['plumbing_electric'] = res_8

    # 逻辑九：装修污染
    res_9 = "建议持续开窗通风"
    vent = get_val('ventilation')
    if hard_budget and area and (hard_budget/area) >= 900:
        res_9 = "建议安装中央新风。"
    elif vent == "晾晒小于1年":
         res_9 = "建议壁挂新风或新风挂机。"
    elif vent == "晾晒大于1年" or vent == "好":
         res_9 = "建议持续开窗通风，可不装新风。"
    result['pollution'] = res_9

    # 逻辑十：流程建议
    res_10 = ""
    accept_design_fee = get_val('budget_can_cover_design', True)
    
    if not accept_design_fee:
        res_10 = TEXT_BLOCKS['flow_3']
    elif score is not None:
        if score >= 5:
            res_10 = TEXT_BLOCKS['flow_1']
        else:
            res_10 = TEXT_BLOCKS['flow_2']
    else:
        res_10 = TEXT_BLOCKS['flow_2']
    
    result['process'] = res_10
    
    return result

def create_docx(logic_result):
    """生成Word文档"""
    doc = Document()
    
    # 样式设置
    style = doc.styles['Normal']
    style.font.name = '微软雅黑'
    style.font.size = Pt(10)
    style.element.rPr.rFonts.set("hint", "微软雅黑")

    # 标题
    heading = doc.add_heading('根据个人情况给出的装修规划指南表', 0)
    heading.alignment = WD_ALIGN_PARAGRAPH.CENTER
    
    # 内容
    items = [
        ("**一、预算是否合理", logic_result['budget_analysis']),
        ("**二、设计费预算建议", logic_result['design_fee']),
        ("**三、设计师档次建议", logic_result['designer_level']),
        ("**四、施工承包方式建议", logic_result['construction_mode']),
        ("**五、施工方建议", logic_result['contractor']),
        ("**六、适合的辅材、主材档次", logic_result['materials']),
        ("**七、零冷水建议：", logic_result['zero_cold_water']),
        ("**八、水电全改半改建议：", logic_result['plumbing_electric']),
        ("**九、入住时间及装修污染解决", logic_result['pollution']),
        ("**十、找设计师、公司的详细流程建议", logic_result['process'])
    ]

    for title, content in items:
        p_title = doc.add_paragraph()
        run_title = p_title.add_run(title)
        run_title.bold = True
        run_title.font.size = Pt(12)
        
        p_content = doc.add_paragraph()
        p_content.add_run("答案建议：\n")
        p_content.add_run(str(content))
        doc.add_paragraph() 

    # 保存到内存
    f = io.BytesIO()
    doc.save(f)
    f.seek(0)
    return f

# ================= UI =================

st.title("☁️ 装修规划 AI 助手 (云端极速版)")
st.markdown("""
**支持手机拍照上传 | 自动分析手写/打印体 | 生成 Word 方案**
*AI 引擎: DeepSeek V3 | OCR 引擎: RapidOCR (Lite Mode)*
""")

uploaded_file = st.file_uploader("点击此处拍照或选择文件", type=['txt', 'docx', 'doc', 'wps', 'pdf', 'jpg', 'jpeg', 'png'])

if uploaded_file is not None:
    # 1. 提取文本
    text_extraction_success = False
    with st.spinner('👀 正在极速识别文字 (RapidOCR)...'):
        raw_text = extract_text_from_file(uploaded_file)
        if raw_text and len(raw_text) > 10 and "OCR 引擎未就绪" not in raw_text:
            text_extraction_success = True
    
    if not text_extraction_success:
        st.error(f"未能提取到有效文字。{raw_text}")
    else:
        # 2. AI 分析
        with st.spinner('🧠 DeepSeek 正在思考规划方案...'):
            structured_data = analyze_data_with_llm(raw_text)
        
        if structured_data:
            # 3. 逻辑推导
            logic_result = logic_engine(structured_data)
            
            # 4. 展示预览
            st.success("✅ 分析完成！")
            
            with st.expander("点击查看规划详情预览"):
                st.write(logic_result)
            
            # 5. 生成下载
            docx_file = create_docx(logic_result)
            
            st.download_button(
                label="📥 点击下载《装修规划指南表》(Word文档)",
                data=docx_file,
                file_name=f"装修规划指南_{datetime.now().strftime('%Y%m%d_%H%M')}.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                type="primary"
            )
            
        else:
            st.error("AI 无法解析数据，请检查 API Key 是否正确，或文件内容是否完整。")
